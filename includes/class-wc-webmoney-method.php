...
class Wc_Webmoney_Method extends WC_Payment_Gateway
{
	...
	public function init_filters()
	{
		...
		add_filter('wc_webmoney_init_form_fields', array($this, 'init_form_fields_purse_wmt' ), 80);
	}
	...
	public function init_options()
	{
		 ...
		 * Set WMT
		 */
		if($this->get_option('purse_wmt_secret') !== '')
		{
			$this->add_purse('USD', $this->get_option('purse_wmt'), $this->get_option('purse_wmt_secret'));
		}
	...
	{
		if(!preg_match('#^[ZREUPT][0-9]{12}$#i', trim($purse)))
		{
			return false;
		}
	...
	public function init_form_fields_purse_wmt($fields)
	{
		$fields['title_wmt'] = array
		(
			'title' => __( 'Purse type: WMT', 'wc-webmoney' ),
			'type' => 'title',
			'description' => __( 'This type of wallet receives money from orders in currency: USD', 'wc-webmoney' ),
		);

		$fields['purse_wmt'] = array
		(
			'title' => __('Purse WMT', 'wc-webmoney'),
			'type' => 'text',
			'description' => __( 'USD webmoney purse to which the buyer has to make a payment for billing in USD.', 'wc-webmoney' ),
			'default' => __('T', 'wc-webmoney')
		);

		$fields['purse_wmt_secret'] = array
		(
			'title' => __('Secret key for WMT', 'wc-webmoney'),
			'type' => 'text',
			'description' => __('Please write Secret key for WMT purse.', 'wc-webmoney'),
			'default' => ''
		);
